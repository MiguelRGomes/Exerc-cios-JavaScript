let sistemaAcademico = () => {
    // declaração das estrtutras de dados
    let vet = []
    let mat = []
    for(let i=0;i<3;i++){
        // para ada aluno
        ...
        vet.push(objeto)
        mat[i] = [] // aloca espaço na memoria para a matriz
    ...
    for(let i=0;i<3;i++){
        ...
        vet[i].media = vet[i].media / 5 // calcula a media
        ...
        if (vet[i].media >= 6){
            ...
            alert(`Aluno ${vet[i.nome]} foi reprovadi`)
        }
    }
    turma = turma / 3
